...
class Protein(object):
    def __init__(self, sequence=None, pose=None):
        ...
        if sequence:
            ...
            self.pose = Pose()
            self.pose.assign(
                pose)  # this is required to copy a pose. Using pose_copy = pose simply creates new pointer to same object.
    ...
    def init_extended(self):
		...
		--------
		Params:
			None, use set_torison to set phi and psi
		Returns:
			None
		"""

        for i in range(1, self.length + 1):  # Need to check: length + 1 or length
            self.set_torsion(position=i, phi=-150, psi=150)
        pass

        pass

    def get_torsion(self, position):
        """
        ...
        phi = self.pose.phi(position)
        psi = self.pose.psi(position)

        return phi, psi
        pass

    def set_torsion(self, position, phi, psi):
        """
        ...
        self.pose.set_phi(position, phi)
        self.pose.set_psi(position, psi)
